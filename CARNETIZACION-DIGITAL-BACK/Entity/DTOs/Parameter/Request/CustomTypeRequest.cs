using System.ComponentModel.DataAnnotations;
using Entity.DTOs.Base;

namespace Entity.DTOs.Parameter.Request
{
    public class CustomTypeRequest : GenericDto
    {
        [StringLength(300, ErrorMessage = "La descripción no puede exceder los 300 caracteres.")]
        [RegularExpression(@"(^$|.*\S.*)", ErrorMessage = "La descripción no puede estar compuesta únicamente por espacios en blanco.")]
        public string? Description { get; set; }

        [Required(ErrorMessage = "El identificador de la categoría de tipo es obligatorio.")]
        [Range(1, int.MaxValue, ErrorMessage = "El identificador de la categoría de tipo debe ser un número entero mayor que 0.")]
        public int TypeCategoryId { get; set; }
    }
}
